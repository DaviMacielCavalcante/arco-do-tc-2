...
def merge_equivalent_evs(raw_entities: dict[str, list[SchemaComponent]]):
...
def walk_and_merge(to_consider: SchemaComponent, sc: SchemaComponent):
...
def homogeneous_arrays_merge(to_consider: ArraySC, sc: ArraySC):
...
def update_references_decide(old: SchemaComponent, new: SchemaComponent, sc: SchemaComponent):
...
class OptionalTagger:
     ...
     def put(self, entity_type_name, schema):
...
def reorder_variation_ids(vars):
...
def sort_by_first_timestamp(vars):
...
def sort_by_last_timestamp(vars):
...
def sort_by_property_number(vars):
...
def null_sort_structural_variations(vars):
     """Não fazer nada (porte de ``NullStructuralVariationSorter.java:9-12``)."""
     return None


def set_optional_properties(variations):
