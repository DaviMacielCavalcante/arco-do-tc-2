...
- [x] ⚠️ **`SimplifyAggrTest` não valida o `EVariationMerger`** (achado resolvido — o teste portado afirma o módulo certo). `fase1_nucleo_inferencia.md` §1.6 diz "1.2 EVariationMerger" e o `INVENTARIO.md` diz "strategies (1.3)"; **os dois erram**. A simplificação `Aggr{V1,V2,V2,…}` → `Aggr{V1,V2}` é feita pelo **`LinkedHashSet` em `SchemaInference.infer(IAJArray)`** (`:237-242`) — módulo **1.2**. O `SimplifyAggr.json` tem `other_names` de tamanho 1/2/4/6 (array de tamanho variável) → é também dado útil para o **#8**.
...
- [x] **Northwind — ✅ FECHADO NA FASE 2.3** (adiamento registrado abaixo; ver `todolist_fase2.md` §2.3). Rodado com a tripla real do `extractors/mongo.py` sobre os 17 arquivos e reconfirmado via `MongoClient` real (Atlas): `compare()` devolve **`equivalent=True`, 15 divergências não-fatais** — assinatura esperada do #8, em `Orders`/`Purchase_orders`/`Products`/`Detail`. ⚠️ **Por decisão do usuário os JSONs não foram vendorizados**, então o resultado está documentado, não versionado como teste. Motivo do adiamento, preservado: reconstruir a tripla à mão não serve — (a) os `count` reais não saem do esquema (confirmado no `model.xmi`, que fecha estruturalmente mas com 1 divergência de count por reconstrução imprecisa); (b) o objetivo do Northwind é **exibir a divergência do #8**, e isso exige a tripla que o oráculo de fato extraiu. **A opção "script leve" (reproduzir o `Helpers.simplify` em Python, como o bloco B) foi investigada e é mais pesada que no bloco B:** o `_id` do Northwind é **inteiro** (`"_id":1`), não ObjectId — cai no #6, então a reprodução teria de replicar o `Helpers` **patcheado** (`0006`), o tratamento de timestamp genérico, e resolver por que o XMI tem uma entidade `_id`/`$oid` se o `_id` é inteiro. Isso é um mini-extrator do caminho-documento — pertence à **Fase 2** (extrator PySpark), não à costura da 1.7. Quando a Fase 2 existir, o golden-master do Northwind (com as 8 não-fatais do #8 **documentadas como resultado**, não corrigidas) fecha o gate de integração.
...
- [x] **Integração:** o pipeline reproduz o XMI-oráculo pelo harness da 0.3 — **mintest com 0 divergências** (`tests/datasets/`, teste permanente). O **Northwind** foi adiado para a Fase 2 (que produz a tripla real) e **fechou na §2.3**: `equivalent=True` com **15 divergências não-fatais**, todas na assinatura do #8. ⚠️ Só o mintest está travado por teste em CI; o Northwind é resultado documentado (JSONs não vendorizados, por decisão).
