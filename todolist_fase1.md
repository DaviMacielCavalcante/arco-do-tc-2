...
**Saída (parcial):** ✅ portados e verdes: `OptionalTest` (3), `TypesTest` (2), `ObjectIdTest` (2, inclui #6), `CountTimestampTest` (4), `SimplifyAggrTest` (2) — `tests/regression/` + fixtures do bloco B em `tests/fixtures/`. **Único pendente:** `J2SchemaSimpleTests` (precisa portar `SchemaPrinter`+`RawSchemaGen`). `RemovePMap`/`RelationshipType` foram pra 1.4b (feitos).
