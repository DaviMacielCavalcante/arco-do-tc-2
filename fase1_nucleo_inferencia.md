...
**Responsável principal:** Davi · **Pré-requisito:** Fase 0 (PyEcore + harness)
...
- [ ] Portar `ObjectSC.meta` (count/timestamps) e o método `combineMetadata` (necessário para a correção do bug #8 — ver 1.2).
...
3. **Colapso de variações** — ao reconhecer uma forma igual a uma variação existente, reutilizá-la. **Correção do bug #8 por construção:** ao reutilizar, **combinar o `meta`** (`retSchema.meta.combineMetadata(novo.meta)`) em vez de descartá-lo — caso contrário, entidades com array de tamanho variável subcontam.
...
> **Onde você corrigiu um bug, ajuste a asserção.** Porte a *estrutura* do teste, mas afirme o valor **corrigido** (#6/#7/#8). Na prática, os casos minúsculos de regressão em geral nem disparam o #8 (que precisa de array de tamanho variável), então a maioria porta sem mudança; só os das áreas de bug pedem o ajuste — e, para o #8, vale **acrescentar** um teste novo com array de tamanho variável afirmando a contagem correta (soma = volume real).
...
2. `infer` recursivo (com a correção do #8) → portar `CountTimestampTest`, `ObjectIdTest`, `TypesTest` + teste novo de array de tamanho variável.
...
`raw.py` + `firsto.py` (modelos intermediários), `strategies.py` (as 6 estratégias + `Null*`), `inference.py` (`SchemaInference`), `builder.py` (`USchemaModelBuilder`/`fillEV`), e a **suíte de testes portada** (regressão JUnit do repo: `CountTimestamp`/`ObjectId`/`Types`/`Optional`/`SimplifyAggr`/`RelationshipTypeToEntityType`/`RemovePMap` + teste novo de contagem do #8 + testes de `__eq__` e por estratégia).

## Riscos da fase

Determinismo (ordenação de campos, igualdade estrutural, ordem de variações) é *load-bearing* — divergência num desses quebra a equivalência; o Inflector tem de casar (resolvido na Fase 0); a igualdade do `ArraySC` que ignora tamanho precisa ser replicada **junto** com a correção do #8 (são duas faces do mesmo ponto).
